...
g_sCmdDesc = "Starts VSCode for a workspace"


####################################################################
def AddArgParseArguments(_xArgParser):

    _xArgParser.add_argument("workspace_path", nargs="?", default=None)
    _xArgParser.add_argument(
        "--develop", dest="develop", action="store_true", default=False
    )
...
def RunCmd(_argsCmd, _lArgs):
    from . import code_show_impl as impl
    ...
    impl.Run(sPathWorkspace=argsSubCmd.workspace_path, bDevelop=argsSubCmd.develop)
