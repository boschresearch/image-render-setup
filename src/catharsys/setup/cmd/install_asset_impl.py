...
###
# File: \install.py
# Created Date: Tuesday, August 2nd 2022, 2:18:33 pm
...
def Run(
    *,
    ...
    if sAssetName is None:
        bDoList = True
    # endif

    if bDoList is False:
        sWsFullName = "image-render-asset-{}".format(sAssetName)

        with res.path(catharsys.setup, "dist") as pathDist:
            pathDistWs = pathDist / "asset"
            ...
            if pathWsSrc is None:
                raise RuntimeError("Asset package '{}' not found".format(sWsFullName))
            # endif

            print(
                "Unpacking asset '{}' to path: {}".format(
        ...
        reDist = re.compile(r"image-render-asset-([^-]+)-(\d+\.\d+\.\d+)\.zip")
        with res.path(catharsys.setup, "dist") as pathDist:
            pathDistWs = pathDist / "asset"
