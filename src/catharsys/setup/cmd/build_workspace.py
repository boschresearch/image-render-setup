...
g_sCmdDesc = "Builds Catharsys workspace"
...
def RunCmd(_argsCmd, _lArgs):
    ...
    from .build_modules_impl import RunWorkspace
    ...
    if argsSubCmd.path is None:
        ...
        pathWorkspace = path.MakeNormPath(argsSubCmd.path)
        if not pathWorkspace.is_absolute():
            pathWorkspace = pathWorkspace.absolute()
        # endif
    # endif

    RunWorkspace(pathWorkspace=pathWorkspace)
